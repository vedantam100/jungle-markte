    ...
    return (
        orderInfo && (
            ...
            <div className="flex justify-center items-center space-y-2 flex-col ">
                <h1 className="text-xl lg:text-2xl font-semibold leading-7 lg:leading-9   text-gray-800">Order #{digitSum(orderInfo.id)} </h1>
                <p className="text-base font-medium leading-6 text-gray-600">{orderInfo.timestamp}</p>
            </div>
            ...
            <div className="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full" key={item.id}>
                <div className="pb-4 md:pb-8 w-full md:w-40">
                    <img className="w-full hidden md:block" src={item.thumbnail} alt="dress" />
                </div>
                <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                    <div className="w-full flex flex-col justify-start items-start space-y-8">
                        ...
                        <div className="flex justify-start items-start flex-col space-y-2">
                            ...
                            <p className="text-sm leading-none text-gray-800">
                                <span className="text-gray-300">Rating: </span> {item.rating}
                            </p>
                        </div>
                    </div>
                    <div className="flex justify-between space-x-8 items-start w-full">
                        <p className="text-base xl:text-lg leading-6">
                        ${(item.price * (1 - item.discountPercentage / 100) ).toFixed(0)} <span className="text-red-300 line-through"> ${item.price}</span>
                        </p>
                        <p className="text-base xl:text-lg leading-6 text-gray-800">{item.quantity}</p>
                        <p className="text-base xl:text-lg font-semibold leading-6 text-gray-800">${(item.price * (1 - item.discountPercentage / 100)* item.quantity ).toFixed(0)}</p>
                    </div>
                </div>
            </div>
                        ))}
                        ...
                        <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 space-y-6   ">
                            ...
                            <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                                <div className="flex justify-between  w-full">
                                    ...
                                    <p className="text-base leading-4 text-gray-600">${orderInfo.cost.mrp}</p>
                                </div>
                                <div className="flex justify-between items-center w-full">
                                    <p className="text-base leading-4 text-gray-800">
                                        Discount <span className="bg-gray-200 p-1 text-xs font-medium leading-3  text-gray-800">WEBSITE</span>
                                    </p>
                                    <p className="text-base leading-4 text-gray-600">-${(orderInfo.cost.mrp).toFixed(0)-(orderInfo.cost.totalprice).toFixed(0)} ({(((orderInfo.cost.mrp).toFixed(0)-(orderInfo.cost.totalprice).toFixed(0))/(orderInfo.cost.mrp).toFixed(0)*100).toFixed(0)}%)</p>
                                </div>
                                <div className="flex justify-between items-center w-full">
                            ...
                            <div className="flex justify-between items-center w-full">
                                ...
                                <p className="text-base font-semibold leading-4 text-gray-600">${(orderInfo.cost.totalprice+8).toFixed(0)}</p>
                            </div>
                        </div>
                    ...
                    <div className="flex  flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0 ">
                        <div className="flex flex-col justify-start items-start flex-shrink-0">
                            ...
                            <div className="flex justify-center  md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                                ...
                                <p className="cursor-pointer text-sm leading-5 text-gray-800">{orderInfo.addresses.email} </p>
                            </div>
                        </div>
                        <div className="flex justify-between xl:h-full  items-stretch w-full flex-col mt-6 md:mt-0">
                            <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row  items-center md:items-start ">
                                <div className="flex justify-center md:justify-start  items-center md:items-start flex-col space-y-4 xl:mt-8">
                                    ...
                                    <p className="w-48 lg:w-full xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{orderInfo.addresses.Address + ", "+orderInfo.addresses.City+ ", "+orderInfo.addresses.PostCode}</p>
                                </div>
